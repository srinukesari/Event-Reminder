		...
		<div class="low">
			...
			<div class="lowleftct">
				<?php
					...
					$u=$_SESSION['imp'];
					// echo $u;
					$sql = "select * from holidays where datee>(select curdate()) order by datee limit 5";
					// $resultc = $conn->query($sql);
					$result = mysqli_query($conn, $sql);
					if (mysqli_num_rows($result) > 0){
						echo '<span style="font-size: 25pt">'.'!!!...Upcoming holidays are...!!!<br><br>'.'</span>';
						while($row = mysqli_fetch_assoc($result)) {echo '<i class="fa fa-bullhorn" style="font-size:32px";></i><i class="fa fa-file-audio-o" style="font-size:32px";></i><span style="font-size: 20pt">'.$row["datee"].'     is   holiday   due   to      '.$row["reason"].'<br>'.'</span>';}}
					else {echo '<span style="font-size: 25pt">'."NO holidays".'</span>';}
					mysqli_close($conn);
					?>
			</div>
		</div>
